...
func (r *ReplicaNode) updateInfo(metaClient MetaClient) error {
	nodes, err := metaClient.ListNodes()
	if err != nil {
		return err
	}
	for _, node := range nodes {
		if node.IPPort() == r.addr {
			r.Status = node.Status
			r.ReplicaCount = node.ReplicaCount
			r.PrimaryCount = node.PrimaryCount
			r.SecondaryCount = node.SecondaryCount
			break
		}
	}
	return nil
